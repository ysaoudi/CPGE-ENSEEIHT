...
procedure Specifier_Et_Tester is
    ...
    function Pgcd (A, B : in Integer) return Integer with
        Pre =>  A > 0 and B > 0,
        Post =>  Pgcd'Result >= 1 --Le résultat que retourne la fonction Pgcd
    ...
    --Non, le corps de la procédure Utiliser_Pgcd est incorrect car il fait
    --appel à la fonction Pgcd avec un paramètre nul ce qui va contre ses
    --pré-conditions.

    -- Que donne son exécution ?
    --Une boucle infinie puisqu'on aura toujours L_A/=L_B si L_A=0
    ...
    procedure Permuter (A, B: in out Integer) with
            Post => A = B'Old and B = A'Old --Valeurs de B et de A respectivement
...
--Exercice8:
    --Le pogramme n'entre pas dans une boucle infinie cette fois-ci
    --mais soulève une exception: le fait qu'une précondition du programme,
    --(ligne 25), n'a pas été respectée.
--Exercie10:
    --La condition Pgcd(105,147)=3 dans Tester_Pgcd n'est pas vraie.
    --On test une procédure par ces post-conditions.
