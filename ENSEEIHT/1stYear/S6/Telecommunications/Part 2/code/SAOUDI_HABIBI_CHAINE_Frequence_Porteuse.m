...
title('Figure 3 : Signal Emis Sans Bruit');
xlabel('Temps en Secondes');
ylabel('Signal Emis');
...
title('Figure 4 : DSP du signal émis');
xlabel('Fréquence en Hz');
ylabel('DSP');


% Retour en bande de base
signal_voieI = x .* cos(2 * pi * fp * T);
signal_voieQ = -j * x .* sin(2 * pi * fp * T);
filtre_passe_bas = 2 * (fc / Fe) * sinc(2 * (fc / Fe) * [-N : N]);
signal_conv = conv(signal_voieI + signal_voieQ, filtre_passe_bas, 'same');
...
figure;
plot(oeil);
title("Figure 6 : Diagramme de l'oeil");
xlabel('Temps en Secondes');
...
for i = 0 : 6
    ...
    bruit_gaussien = sqrt(Puissance_bruit)*N;
    y = x + bruit_gaussien;
    signal_phaseI = y .* cos(2 * pi * fp * T);
    signal_phaseQ = -1i * y .* sin(2 * pi * fp * T);
    y = signal_phaseI + signal_phaseQ; 
...
title('Figure 7 : Comparaison des les TEB');
