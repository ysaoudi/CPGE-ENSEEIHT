...
public class IncrMes {
    ...
    static void lancer(int nbA, Incrementeur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMes.activites[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMes.activites[i].start(); // permet de lancer l'instance de thread auquel elle est appliquee
        }
    }

    static void finir() {
        // Attente de la terminaison des differents threads lances
        for (int i = 0; i < IncrMes.activites.length ; i++) {
            try {
                IncrMes.activites[i].join(); // permet d'attendre la terminaison de l'instance de thread auquel elle est appliquee
            }
    ...
    public static void main(String[] args) {
        ...
        // Idee = retenir une moyenne des dernieres mesures.
            // Boucle sequentielle : NB_IT x NB_IT_INTERNES x nbActivites iterations
             ...
             for (long li = 0; li < nbActivites*IncrMes.NB_IT; li++) {
...
class IncrementeurNonSync implements Incrementeur {
    ...
    public void run() {
        ...
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur avant : " + IncrMes.cpt);
        this.incr();
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur après : " + IncrMes.cpt);
    }
}