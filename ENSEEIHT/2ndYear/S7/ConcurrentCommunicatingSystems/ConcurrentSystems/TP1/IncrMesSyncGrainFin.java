...
public class IncrMesSyncGrainFin {
    ...
    static void lancer(int nbA, Incrementeur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMesSyncGrainFin.activites[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMesSyncGrainFin.activites[i].start(); // permet de lancer l'instance de thread auquel elle est appliquee
        }
    }

    static void finir() {
        // Attente de la terminaison des differents threads lances
        for (int i = 0; i < IncrMesSyncGrainFin.activites.length ; i++) {
            try {
                IncrMesSyncGrainFin.activites[i].join(); // permet d'attendre la terminaison de l'instance de thread auquel elle est appliquee
            }
    ...
    public static void main(String[] args) {
        ...
        // Idee = retenir une moyenne des dernieres mesures.
            // Boucle sequentielle : NB_IT x NB_IT_INTERNES x nbActivites iterations
             ...
             for (long li = 0; li < nbActivites*IncrMesSyncGrainFin.NB_IT; li++) {
                for (long j = 1; j<=IncrMesSyncGrainFin.NB_IT_INTERNES; j++) {
                    IncrMesSyncGrainFin.cpt=IncrMesSyncGrainFin.cpt+j/j;
                    // j/j pour tenter de dejouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesSyncGrainFin.Attente_ms, IncrMesSyncGrainFin.Attente_nano);
...
class IncrementeurNonSync implements Incrementeur {
     ...
     */
    public void incr() {
        for (long i = 0L; i < IncrMesSyncGrainFin.NB_IT; i++) {
    ...
    public void run() {
        ...
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur avant : " + IncrMesSyncGrainFin.cpt);
        this.incr();
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur après : " + IncrMesSyncGrainFin.cpt);
    }
}