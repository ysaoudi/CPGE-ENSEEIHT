...
public class IncrMesSyncGrosGrain {
    ...
    static void lancer(int nbA, Incrementeur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMesSyncGrosGrain.activites[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMesSyncGrosGrain.activites[i].start(); // permet de lancer l'instance de thread auquel elle est appliquee
        }
    }

    static void finir() {
        // Attente de la terminaison des differents threads lances
        for (int i = 0; i < IncrMesSyncGrosGrain.activites.length ; i++) {
            try {
                IncrMesSyncGrosGrain.activites[i].join(); // permet d'attendre la terminaison de l'instance de thread auquel elle est appliquee
            }
    ...
    public static void main(String[] args) {
        ...
        // Idee = retenir une moyenne des dernieres mesures.
            // Boucle sequentielle : NB_IT x NB_IT_INTERNES x nbActivites iterations
             ...
             for (long li = 0; li < nbActivites*IncrMesSyncGrosGrain.NB_IT; li++) {
                for (long j = 1; j<=IncrMesSyncGrosGrain.NB_IT_INTERNES; j++) {
                    IncrMesSyncGrosGrain.cpt=IncrMesSyncGrosGrain.cpt+j/j;
                    // j/j pour tenter de dejouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesSyncGrosGrain.Attente_ms, IncrMesSyncGrosGrain.Attente_nano);
...
class IncrementeurNonSync implements Incrementeur {
    ...
    public void incr() {
        synchronized(mutex)
        {
            for (long i = 0L; i < IncrMesSyncGrosGrain.NB_IT; i++) {
    ...
    public void run() {
        ...
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur avant : " + IncrMesSyncGrosGrain.cpt);
        this.incr();
        System.out.println("Thread " + Thread.currentThread().getName() + " - compteur après : " + IncrMesSyncGrosGrain.cpt);
    }
}