...
interface Incrementeur extends Runnable {
    void incr();
    /*
     * Boucle d'incrementation effectuee par un processus (thread). Le
     * principe/objectif est de realiser cette boucle, comportant un meme nombre
...
public class IncrMesSyncGrainMoyen {
    ...
    static void lancer(int nbA, Incrementeur r) {
        ...
        cpt = 0L;
        /*
         * Creation et lancement des threads. Chaque thread execute le meme code, qui
         * est la methode run() d'une classe implantant un Incrementeur (autrement dit
         * un schema de realisation particulier de la boucle dincrementation)
         */

        for (int i = 0; i < nbA; i++) {
            try {
                IncrMesSyncGrainMoyen.activites[i] = new Thread(r, "t" + i);
            } catch (Exception e) {
                System.out.println(e);
            }
            IncrMesSyncGrainMoyen.activites[i].start(); // permet de lancer l'instance de thread auquel elle est
                                                        // appliquee
        }
    }
    ...
    static void finir() {
        ...
        for (int i = 0; i < IncrMesSyncGrainMoyen.activites.length; i++) {
            try {
                IncrMesSyncGrainMoyen.activites[i].join(); // permet d'attendre la terminaison de l'instance de thread
    ...
    public static void main(String[] args) {
        ...
        for (int it = 1; it <= nbMesures; it++) { //
            // Mesure iteree, pour eviter des ecarts trop importants sur les premieres
            // mesures
            ...
            for (long li = 0; li < nbActivites * IncrMesSyncGrainMoyen.NB_IT; li++) {
                for (long j = 1; j <= IncrMesSyncGrainMoyen.NB_IT_INTERNES; j++) {
                    IncrMesSyncGrainMoyen.cpt = IncrMesSyncGrainMoyen.cpt + j / j;
                    // j/j pour tenter de dejouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesSyncGrainMoyen.Attente_ms, IncrMesSyncGrainMoyen.Attente_nano);
...
class IncrementeurNonSync implements Incrementeur {
    ...
    /*
     * Exemple d'incrementeur : -la methode incr effectue les incrementations sans
     * gerer les conflis d'acces au compteur - la methode run() appelle incr()
     */
    public void incr() {

        for (long i = 0L; i < IncrMesSyncGrainMoyen.NB_IT; i++) {
    ...
    public void run() {
        ...
        System.out.println(
                "Thread " + Thread.currentThread().getName() + " - compteur avant : " + IncrMesSyncGrainMoyen.cpt);
        this.incr();
        System.out.println(
                "Thread " + Thread.currentThread().getName() + " - compteur après : " + IncrMesSyncGrainMoyen.cpt);
    }
}