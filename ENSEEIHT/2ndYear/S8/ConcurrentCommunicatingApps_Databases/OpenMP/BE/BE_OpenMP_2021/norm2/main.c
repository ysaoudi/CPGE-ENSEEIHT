 ...
 * Execution Time:   (for n = 10^5)
 *                  1 thread  : 0.1 ms (all)
 *                  2 threads : 0.4 ms (seq) / 0.3 ms (red) / 0.1 ms (no_red)
 *                  4 threads : 0.3 ms (seq) / 4.7 ms (red) / 10.8 ms (no_red)
 * Conclusion: 
 *            Depending on the size of the array, adding more threads may actually
 *            get in the way of what we are trying to achieve, which is a parallelization
 *            faster than the sequential version. One has to find the appropriate amount
 *            of threads to use depending on the size of the array. Here, going above 2 threads
 *            for n = 10^5 is disadvantageous.
 **/
