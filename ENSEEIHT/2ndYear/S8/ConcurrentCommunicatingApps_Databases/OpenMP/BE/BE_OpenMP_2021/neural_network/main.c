...
void parallel_nn_tasks(layer *Layers, data *Datas, int N, int m, int L){
  ...
  #pragma omp parallel
  {
    #pragma omp single
    {
      #pragma omp task depend(inout:Datas) depend(in:Layers[l])
      {
        for(l=0; l<L; l++){
          for(i=0; i<N; i++)
            for(j=0; j<N; j++){
      ...
      #pragma omp task depend(in:Layers[l], Datas[l]) depend(inout: Datas[l+1])
      {  
        for(i=0; i<N; i++)
        {
          block_bias_act(Layers[l].b[i], Datas[l+1].X[i], m);
 ...
 * Execution Time: (n = 2000, m = 100, L = 10)
 *  1 Thread : 743.15 ms
 *  2 Threads : 375.8 ms (loop) / 736.93 ms (tasks)
 *  4 Threads : 216.19 ms (loop) / 661.54 ms (tasks)
 **/